      ...
      >
        <div class="header">
          <h5
            ...
            @click="$store.commit('set-active-route', index)"
          >
          ...
          <div>
            <button
              ...
              @click="$store.dispatch('reverse', index)"
            >
              Reverse
            </button>
            <button
              class="btn-small lime"
              @click="$store.commit('duplicate', index)"
            >
              Duplicate
            </button>
            <button
              class="btn-small cyan"
              @click="$store.dispatch('join', index)"
            >
              Join Down
            </button>
            <button
              class="btn-small red darken-1"
              @click="$store.dispatch('remove', index)"
            >
              Delete
            </button>
            <button
...
export default {
  ...
  methods: {
    exportRoute(index) {
      const routeName = this.$store.state.routes[index].gpx.metadata[0].name[0];
      const blob = new Blob([this.$store.getters.routeStrings[index]], {type: "text/plain;charset=utf-8"});
