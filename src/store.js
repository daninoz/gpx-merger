...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    ["delete-point"](state, payload) {
      state.routes[payload.routeIndex].gpx.trk[0].trkseg[0].trkpt.splice(
        payload.pointIndex,
        1
      );
    },
    ...
    ["delete-after-point"](state, payload) {
      const total =
        state.routes[payload.routeIndex].gpx.trk[0].trkseg[0].trkpt.length;
      state.routes[payload.routeIndex].gpx.trk[0].trkseg[0].trkpt.splice(
        payload.pointIndex + 1,
        total - payload.pointIndex
      );
    },
    ...
    ["join"](state, routeIndex) {
      ...
      const gpxBPoints =
        state.routes[routeIndex + 1].gpx.trk[0].trkseg[0].trkpt;
    ...
    ["go-prev-point"](state) {
      ...
      if (activePointIndex > 0) {
        ...
        const prevRouteLastPoint =
          state.routes[activeRouteIndex - 1].gpx.trk[0].trkseg[0].trkpt.length -
          1;
    ...
    ["go-next-point"](state) {
      ...
      const activeRouteTotalPoints =
        state.routes[activeRouteIndex].gpx.trk[0].trkseg[0].trkpt.length;
      const nextPoint = {};
  ...
  actions: {
    ...
    ["join"]({ commit, state }, payload) {
      const totalRoutePoints =
        state.routes[payload].gpx.trk[0].trkseg[0].trkpt.length;
